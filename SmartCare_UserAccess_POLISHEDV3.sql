...
select staffid,firstname,lastname into #problemsolving from Staff S
where S.Active='Y'
and ISNULL(S.RECORDDELETED,'N')='N'
AND S.Lastvisit >= '2022-01-01'

select * from #problemsolving
where StaffId=31013

drop table #problemsolving
...
select S.*,SR1.RoleId AS 'AdministratorROLE'
from Staff S
LEFT JOIN StaffRoles SR1 on S.Staffid = SR1.Staffid and SR1.RoleId=50104 
where S.Active='Y'
...
where globalcodeid=50104

select staffroles.*,globalcodes.codename from staffroles
left join globalcodes on staffroles.roleid=globalcodes.GlobalCodeId
where staffid=31013
and roleid=43667
...
select StaffId,USERCODE,LastName,FirstName,Active FROM Staff
where staffid in (1113)

select StaffId,USERCODE,LastName,FirstName,Active into #tempadditions FROM Staff
where staffid in (1112,1113,23,682,1912,12,31013)
...
SELECT STAFFID,USERCODE,LastName,FirstName,Active FROM #COUNTDOWN
UNION 
