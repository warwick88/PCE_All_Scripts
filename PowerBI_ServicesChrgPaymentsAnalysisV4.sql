...
/*
		Data Sources Created / Still needing creation

		Facts: ksp_Rpt_SrvRevDashboard_ServiceFacts     
			   ksp_Rpt_SrvRevDashboard_Charges
			   ksp_Rpt_SrvRevDashboard_ARLedger
			   ksp_Rpt_SrvRevDashboard_ServiceClients
			   ksp_Rpt_SrvRevDashboard-ServiceClinicians
			   ksp_Rpt_SrvRevDashboard_ClaimLineFacts
			   
			   TEST AREA
			   EXEC ksp_Rpt_SrvRevDashboard_ServiceFacts <-DONE Looks good, cleaned columns
			   EXEC ksp_Rpt_SrvRevDashboard_Charges <--DONE Looks good, cleaned columns
			   EXEC ksp_Rpt_SrvRevDashboard_ARLedger <--Pretty close to being done, might not be worth writing grab just to remove row Identifier
			   EXEC ksp_Rpt_SrvRevDashboard_ServiceClients <--DONE Looks good, cleaned columns
			   EXEC ksp_Rpt_SrvRevDashboard_ServiceClinicians <-- Let's review
			   ksp_Rpt_SrvRevDashboard_ClaimLineFacts

		Dimensions: kv_SrvRevDashboard_Programs VIEW


	Notes 5/4/2022: STORED PROC Created for Services Facts:
	from meeting with Team: CREATE VIEW [dbo].[kv_SrvRevDashboard_Programs] This view is now all set and available for use.
	Notes: So 
